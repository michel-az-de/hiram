...
public sealed class PushConsumerWorker : BackgroundService
{
    private static readonly TimeSpan RequeueBackoff = TimeSpan.FromSeconds(2);

    private readonly RabbitMqConnection _connection;
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ILogger<PushConsumerWorker> _logger;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        await _channel.BasicConsumeAsync(HiramTopology.PushQueue, autoAck: false, consumer, stoppingToken);
    ...
    private async Task OnReceivedAsync(object sender, BasicDeliverEventArgs args)
    {
        using var activity = HiramDiagnostics.Messaging.StartActivity(
            "consume push", ActivityKind.Consumer, ExtractContext(args.BasicProperties.Headers));
        ...
        catch (PoisonMessageException ex)
        {
            ...
            _logger.LogWarning(ex, "Parking poison push message {DeliveryTag}", args.DeliveryTag);
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Transient failure on push message {DeliveryTag}, requeuing", args.DeliveryTag);
