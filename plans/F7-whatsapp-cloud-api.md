...
2. **Consent obrigatório fiado** (`feat: enforce whatsapp consent before dispatch`). Default consciente do canal em `ConsentPolicy` (WhatsApp nega sem opt-in em toda categoria), `ChannelResolver` chamado no caminho do WhatsApp, supressão com `MarkSuppressed` e registro auditável da decisão. Remover o item #37 da allowlist do `DependencyInjectionOrphanTests`, que o `Allowlist_HasNoStaleEntries` passa a exigir. Testes unitários do default e de integração da supressão.
