...
1. [ ] Servir `site/` sob `/learn` na Hiram.Api, gated a Development, com `site/learn/index.html` (passo 1).
2. [ ] Hub pedagógico dos dez conceitos, diagrama E2E e console híbrido Live mais palco (passos 2 a 5, 7).
3. [ ] Endpoint `POST /demo/bootstrap` dev-only, idempotente, guardado por `X-Admin-Key` (passo 6).
